...
class Character(pygame.sprite.Sprite):
  def __init__(self, position, groups, frames, obstacles):
    ...
    #silly sounds
    self.bluh1 = pygame.mixer.Sound("assets/audio/pluh.wav")
    self.bluh2 = pygame.mixer.Sound("assets/audio/awhh.wav")
      ...
      for obstacle in self.obstacles:
        if obstacle.rect.colliderect(self.hitbox):
          if self.direction_vector["x"] > 0:
            ...
            pygame.mixer.Sound.play(self.bluh1)
          elif self.direction_vector["x"] < 0:
            self.hitbox.left = obstacle.rect.right
            pygame.mixer.Sound.play(self.bluh2)
    if direction == 'y':
      for obstacle in self.obstacles:
        if obstacle.rect.colliderect(self.hitbox):
